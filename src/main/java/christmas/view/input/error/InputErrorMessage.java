package christmas.view.input.error;

import static christmas.view.constant.CharacterSymbol.BLANK;

public class InputErrorMessage {

    private static final String RE_INPUT = "다시 입력해 주세요.";
    public static final String VISIT_DATE_ERROR = "유효하지 않은 날짜입니다." + BLANK.getLiteral() + RE_INPUT;
    public static final String ORDER_ERROR = "유효하지 않은 주문입니다." + BLANK.getLiteral() + RE_INPUT;

}
